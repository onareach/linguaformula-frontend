...
function TermQuizContent() {
  ...
  useEffect(() => {
    if (!id || !process.env.NEXT_PUBLIC_API_URL) {
      setError("Missing term ID or API URL.");
      setLoading(false);
      return;
    }
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/terms/${id}/questions`)
      .then((res) => {
      ...
      case "word_problem":
        return (
          <WordProblemQuestion
            stem={current.stem}
            value={wordAnswer}
            onChange={setWordAnswer}
            inputId={`term-quiz-word-${current.question_id}`}
          />
        );
      case "multipart":
...
export default function TermQuizPage() {
  return (
